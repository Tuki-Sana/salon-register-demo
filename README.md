...
- **モーダル**: 緑ヘッダーに白字・中央揃えのタイトルで「今何の設定をしているか」を明示。閉じるボタンは `position: absolute` で右端固定し、タイトルを完全中央揃えに統一。コンテンツ部分は `flex: 1; min-height: 0; overflow-y: auto` で高さを制御し、モーダル自体は `max-height` で画面内に収める。
